"""
DESAFIO 076:
Crie um programa que tenha uma tupla única com nomes de produtos e seus 
respectivos preços, na sequência. No final, mostre uma listagem de 
preços, organizando os dados em forma tabular.
"""

# Tupla única com nomes e preços intercalados
listagem = ('Lápis', 1.75, 'Borracha', 2, 'Caderno', 15.90, 
            'Estojo', 25, 'Transferidor', 4.20, 'Compasso', 9.99, 
            'Mochila', 120.32, 'Canetas', 22.30, 'Livro', 34.90)

# Cabeçalho da listagem
print('-' * 40)
print(f'{"LISTAGEM DE PREÇOS":^40}')
print('-' * 40)

# Otimização: percorrendo a tupla de 2 em 2 (item e preço)
# O uso de 'range' com salto de 2 é mais limpo que usar o 'if pos % 2 == 0'
for i in range(0, len(listagem), 2):
    nome = listagem[i]
    preco = listagem[i+1]
    
    # Formatação tabular:
    # :.<30 preenche com pontos até 30 espaços
    # :>7.2f formata o float com 2 casas decimais e alinha à direita
    print(f'{nome:.<30}R${preco:>7.2f}')

print('-' * 40)